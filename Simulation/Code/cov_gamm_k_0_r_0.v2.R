						...
						for (sig.strength in sig.strengths) {
							...
							if (struct.type == 'Covariate') {
								...
								try({
											...
											formulas <- paste0("x")
											adapt1.res <- adapt_glm(x = x.df, pvals = pvalue, pi_formulas = formulas, mu_formulas = formulas, verbose = 
															list(print = FALSE, fit = FALSE, ms = FALSE))
