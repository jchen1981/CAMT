...
for (prefix in c('cov_norm_k_0_r_0_3.v2', 'cov_norm_k_0_r_0_4.v2')) {	
	cat(prefix)
...
# Plot - Annonation, theme
	...
	ltys <-  c('Oracle'=1, 'CAMT'=1, 'AdaPT'= 2,'IHW'= 1,  'FDRregT'=2, 'FDRregE'=3, 'SABHA'=1, 'BL'=2,'BH'=2, 'ST'=1)
	shapes <- c('Oracle'=17,'CAMT'=19,   'AdaPT'=3,  'IHW'=2, 'FDRregT'=0, 'FDRregE'=2,'SABHA'=4, 'BL'=6, 'BH'=15, 'ST'=5)
	...
	for (Struct in struct.types) {
		...
		if (Struct == 'Covariate') {
			algs2 <-  c('Oracle',  'CAMT',  'IHW', 'FDRregT', 'FDRregE', 'AdaPT', 'SABHA',  'BL', 'BH', 'ST')
		...
		for (Dist in sig.dists) {
			...
			pdf(paste0(prefix, "_combined_curve.r2.E.pdf"), width=8, height=9.5)
