					...
					for (sig.density in sig.densities) {
						...
						for (sig.strength in sig.strengths) {
							...
							data <- simulate.data(covariate.strength = prior.strength, sig.density=sig.density, sig.strength = sig.strength,
									sig.dist = sig.dist, feature.no = feature.no, f1.sd = 0.5,
							...
							if (struct.type == 'Covariate') {
								...
								try({
											cat('.')
											res.obj[['CAMT']] <- list(fdr = camt.fdr(pvals = pvalue, pi0.var = x1, f1.var = ns(x2, 6), 
								...
								try({
											...
											mu_formulas <- paste0("ns(x2, 6)")
											adapt1.res <- adapt_glm(x = x.df, pvals = pvalue, pi_formulas = pi_formulas, mu_formulas = mu_formulas, verbose = list(print = FALSE, fit =
																	FALSE, ms = FALSE))
